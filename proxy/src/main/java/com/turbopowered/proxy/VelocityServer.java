...
package com.turbopowered.proxy;
...
import com.turbopowered.api.command.BrigadierCommand;
import com.turbopowered.api.event.proxy.ProxyInitializeEvent;
import com.turbopowered.api.event.proxy.ProxyPreShutdownEvent;
import com.turbopowered.api.event.proxy.ProxyReloadEvent;
import com.turbopowered.api.event.proxy.ProxyShutdownEvent;
...
import com.turbopowered.api.proxy.ProxyServer;
...
import com.turbopowered.proxy.command.builtin.SendCommand;
import com.turbopowered.proxy.command.builtin.ServerCommand;
import com.turbopowered.proxy.command.builtin.ShutdownCommand;
...
import com.turbopowered.proxy.network.ConnectionManager;
import com.turbopowered.proxy.plugin.VelocityPluginManager;
import com.turbopowered.proxy.plugin.loader.VelocityPluginContainer;
...
import com.turbopowered.proxy.protocol.util.FaviconSerializer;
import com.turbopowered.proxy.protocol.util.GameProfileSerializer;
import com.turbopowered.proxy.scheduler.VelocityScheduler;
import com.turbopowered.proxy.server.ServerMap;
import com.turbopowered.proxy.util.AddressUtil;
import com.turbopowered.proxy.util.ClosestLocaleMatcher;
import com.turbopowered.proxy.util.ResourceUtils;
import com.turbopowered.proxy.util.VelocityChannelRegistrar;
import com.turbopowered.proxy.util.ratelimit.Ratelimiter;
...
public class VelocityServer implements ProxyServer, ForwardingAudience {
  ...
  public ProxyVersion getVersion() {
    // COMPATIBILITY MODE:
    // Many plugins check if version.getName().equals("Velocity").
    // If we change it to "TurboProxy", they break.
    // So we LIE to the plugins about the name, but allow branding in Vendor/Version.
    
    Package pkg = VelocityServer.class.getPackage();
    String implName = "Velocity"; // HARDCODED FOR COMPATIBILITY
    String implVersion;
    String implVendor;
    if (pkg != null) {
      implVersion = MoreObjects.firstNonNull(pkg.getImplementationVersion(), "<unknown>") + " (TurboProxy)";
      implVendor = "TurboProxy Team"; 
    } else {
      implVersion = "<unknown> (TurboProxy)";
      implVendor = "TurboProxy Team";
    }
